...
const allServersTextArea = document.querySelector('#servers-list');
...
function start() {
  ...
  const myIceServers = JSON.parse(allServersTextArea.value);
  console.log('my ice servers', myIceServers);


  // Create a PeerConnection with no streams, but force a m=audio line.
  const config = {
    iceServers: iceServers.concat(myIceServers),
  ...
  myIceServers.forEach(server => {
    let pc = new RTCPeerConnection(config);
    pc.onicecandidate = (event) => {
      iceCallback(event, pc, server);
    };
    pc.onicegatheringstatechange = () => {
      gatheringStateChange(pc);
    };
    pc.onicecandidateerror = iceCandidateError;
    pc.createOffer(
        offerOptions
    ).then((arg) => {
      gotDescription(arg, pc);
    }, (err) => {        
        noDescription(err, pc);
      }
    );
    connections.push(pc);
  })
...
function gotDescription(desc, pc) {
...
function gatheringStateChange(pc) {
